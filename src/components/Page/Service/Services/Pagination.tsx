...
const Pagination = memo(({ totalItems, itemsPerPage }: PaginationProps) => {
  ...
  const getCurrentPage = () => {
    ...
    if (!isNaN(pageParam) && pageParam >= 1 && pageParam <= totalPages) return pageParam;
    return 1;
  };
  ...
  const btnBase = "p-2 rounded transition-colors duration-200 disabled:opacity-40 disabled:cursor-not-allowed";
  ...
  const renderPageNumbers = () => {
    const pages = totalPages <= 5
      ...
      : (() => {
          ...
          for (let i = Math.max(2, currentPage - 1); i <= Math.min(totalPages - 1, currentPage + 1); i++) list.push(i);
    ...
    return pages.map((page, index) =>
      page < 0 ? (
        <span key={`ellipsis-${index}`} className="px-2" style={{ color: 'var(--text-muted)' }}>…</span>
      ) : (
        <button
          key={page}
          ...
          style={currentPage === page
            ...
            : { backgroundColor: 'var(--bg-raised)', color: 'var(--text-muted)' }
          }
        >
          {page}
        </button>
  ...
  return (
    <div className="flex md:justify-end justify-center items-center gap-1.5 mt-6 flex-wrap">
      {[
        ...
        { icon: FiChevronLeft,   page: currentPage - 1,  disabled: currentPage === 1 },
      ...
      {[
        { icon: FiChevronRight,  page: currentPage + 1, disabled: currentPage === totalPages },
        { icon: FiChevronsRight, page: totalPages,       disabled: currentPage === totalPages },
      ].map(({ icon: Icon, page, disabled }) => (
        <button
          key={page}
          onClick={() => handlePageChange(page)}
