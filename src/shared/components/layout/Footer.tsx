...
import {
  ...
  FaTiktok, FaYoutube, FaMapMarkerAlt, FaEnvelope, FaPhone, FaClock,
} from 'react-icons/fa';
import logo from '../../../assets/images/logo.png';
import { useAppSelector } from '../../../app/hooks';
import { selectServices, selectServicesStatus } from '../../../store/servicesSlice';
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div className="flex flex-col gap-5">
            <Link to="/"><img src={logo} alt="Tensor Labs" className="h-10 w-auto object-contain" loading="lazy" /></Link>
          ...
          <div className="flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-2.5">
              {isLoading
                ...
                : serviceLinks.map((s) => (
                    <li key={s.link}>
                      <Link to={s.link} className="text-sm text-slate-500 hover:text-sky-400 transition-colors duration-200">
                        {s.title}
                      </Link>
                    </li>
                  ))}
          ...
          <div className="flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-2.5">
              {companyLinks.map(({ title, link }) => (
                <li key={link}>
                  <Link to={link} className="text-sm text-slate-500 hover:text-sky-400 transition-colors duration-200">{title}</Link>
                </li>
              ))}
            </ul>
          </div>
            ...
            <ul className="flex flex-col gap-3">
              {contactInfo.map(({ icon: Icon, text, href }) => (
                <li key={text} className="flex items-start gap-3">
                  ...
                  {href ? (
                    <a href={href} className="text-sm text-slate-500 hover:text-sky-400 transition-colors duration-200 leading-snug">{text}</a>
                  ) : (
