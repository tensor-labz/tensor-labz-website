...
export async function fetchSheet<T>(
  ...
  if (!response.ok) throw new Error(`HTTP ${response.status} fetching ${sheetName}`);
  const result = await response.json();
  if (!Array.isArray(result?.data)) throw new Error(`Unexpected response format from ${sheetName}`);
