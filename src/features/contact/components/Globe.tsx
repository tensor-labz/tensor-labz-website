    ...
    const init = () => {
      ...
      const globe = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({
          color: ACCENT, wireframe: true, transparent: true, opacity: 0.12,
        }),
      );
      scene.add(globe);
      ...
      const dots = new THREE.Points(
        dotGeo,
        new THREE.PointsMaterial({ color: ACCENT, size: 0.014, transparent: true, opacity: 0.9 }),
      );
      scene.add(dots);
      ...
      /* arc lines */
      const arcMat   = new THREE.LineBasicMaterial({ color: ACCENT, transparent: true, opacity: 0.35 });
      const arcGroup = new THREE.Group();
      for (let i = 0; i < ARC_PAIRS; i++) {
        const geo = new THREE.BufferGeometry();
        geo.setAttribute('position', new THREE.Float32BufferAttribute(arcPoints(randomOnSphere(), randomOnSphere()), 3));
