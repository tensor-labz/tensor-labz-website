...
const MobileSlider = memo(({ projects }: { projects: ProjectItem[] }) => {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => { setDir(1); setIndex((p) => (p + 1) % projects.length); }, 6000);
  ...
  const variants = {
    ...
    center: { x: 0, opacity: 1, transition: { duration: 0.45, ease: EASE_EXPO } },
    exit:  (d: number) => ({ x: d > 0 ? -260 : 260, opacity: 0, transition: { duration: 0.35 } }),
  };
      ...
      <div className="overflow-hidden rounded-xl">
        <AnimatePresence initial={false} custom={dir} mode="wait">
          ...
          >
            ...
            {/* Live badge */}
            <div className="absolute top-3 left-3 flex items-center gap-1.5 px-2 py-0.5 rounded
              border border-accent/40 bg-black/60 backdrop-blur-sm">
              <span className="w-1.5 h-1.5 rounded-full bg-accent animate-pulse" />
              <span className="text-[9px] font-mono tracking-widest uppercase text-accent">Project</span>
            </div>
            {/* Title strip */}
            <div className="px-4 py-3 flex items-center justify-between gap-2 relative z-10">
              <h5 className="text-sm font-semibold font-display text-fg truncate">{p.title}</h5>
              <ReactIcon name="FiArrowRight" size={14} className="text-accent shrink-0" />
            </div>
          </motion.div>
        </AnimatePresence>
      </div>
